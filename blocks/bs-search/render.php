...
if (isset($attributes)) {
    if (isset($attributes['showLabel']) && true === $attributes['showLabel']) {
        $searchLabel = __('Search', 'bootstrap-basic-fse');
        ...
        $field_markup .= '<label class="form-label" for="bootstrap-basic-fse-blocks-bs-search-input">' . wp_kses_post($searchLabel) . '</label>';
        $field_markup .= '</div>';
...
$input_field = '<input id="bootstrap-basic-fse-blocks-bs-search-input" class="form-control" type="search" name="s" value="' . get_search_query() . '" placeholder="' . esc_attr($placeholderText) . '" required>';

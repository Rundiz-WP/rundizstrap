...
if (!class_exists('\\BootstrapBasicFSE\\Libraries\\BootstrapNavbarNavigationWalker')) {
    /**
    ...
    class BootstrapNavbarNavigationWalker
    {
        ...
        private static function buildItemFromHomeLinkBlock(array $block): array
        {
            ...
            $label = ($attrs['label'] ?? '');
            if ('' === trim($label)) {
                $label = __('Home', 'bootstrap-basic-fse');
            }
        ...
        public function render(array $items, int $depth = 0): string
        {
            ...
            foreach ($items as $item) {
                $label = (isset($item['label']) && '' !== trim($item['label']) ? $item['label'] : __('(Untitled)', 'bootstrap-basic-fse'));

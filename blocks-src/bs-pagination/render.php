 ...
 * @return string
 */
function bootstrap_basic_fse_block_bsPagination_generateOutputHTML(array $attributes, string $content, \WP_Block $block, array $pageResult): string
{
 ...
 * @return array
 */
function bootstrap_basic_fse_block_bsPagination_renderPageNext(array $attributes, string $content, \WP_Block $block): array
{
    ...
    $label = $label_text;
    $wrapper_attributes .= ' aria-label="' . __('Next', 'bootstrap-basic-fse') . '"';
    ...
    if (isset($block->context['query']['inherit']) && $block->context['query']['inherit']) {
        ...
        if ('' === $content || !is_scalar($content)) {
            $content = sprintf(
                '<a class="wp-block-bootstrap-basic-fse-blocks-bs-pagination page-link disabled" %1$s>%2$s</a>',
                $wrapper_attributes,
                $label
            );
        }
        ...
        if ($block_query && $block_max_pages !== $page) {
            $content = sprintf(
                '<a class="wp-block-bootstrap-basic-fse-blocks-bs-pagination page-link" href="%1$s" %2$s>%3$s</a>',
    ...
    if ($enhanced_pagination && isset($content)) {
        ...
        if ($p->next_tag(
            [
                'tag_name' => 'a',
                'class_name' => 'wp-block-bootstrap-basic-fse-blocks-bs-pagination',
            ]
        )) {
    ...
    return [$content];
}// bootstrap_basic_fse_block_bsPagination_renderPageNext
 ...
 * @return array
 */
function bootstrap_basic_fse_block_bsPagination_renderPageNumbers(array $attributes, string $content, \WP_Block $block): array
{
    ...
    return \BootstrapBasicFSE\WPOverride\GeneralTemplate::paginate_links($paginate_args);
}// bootstrap_basic_fse_block_bsPagination_render_pageNumbers
 ...
 * @return array
 */
function bootstrap_basic_fse_block_bsPagination_renderPagePrevious(array $attributes, string $content, \WP_Block $block): array
{
    $page_key = (isset($block->context['queryId']) ? 'query-' . $block->context['queryId'] . '-page' : 'query-page');
    $enhanced_pagination = isset($block->context['enhancedPagination']) && $block->context['enhancedPagination'];
    $max_page = (isset($block->context['query']['pages']) ? (int) $block->context['query']['pages'] : 0);
    ...
    $label = $label_text;
    $wrapper_attributes .= ' aria-label="' . __('Previous', 'bootstrap-basic-fse') . '"';
    ...
    return [$content];
}// bootstrap_basic_fse_block_bsPagination_renderPagePrevious
...
if (true === $showPreviousNext) {
    $pageResult = array_merge(
        $pageResult, 
        bootstrap_basic_fse_block_bsPagination_renderPagePrevious(($attributes ?? []), ($content ?? ''), ($block ?? new \WP_Block))
    );
...
if (isset($attributes['showPageNumbers']) && true === $attributes['showPageNumbers']) {
    $pageResult = array_merge(
        $pageResult, 
        bootstrap_basic_fse_block_bsPagination_renderPageNumbers(($attributes ?? []), ($content ?? ''), ($block ?? new \WP_Block))
    );
...
if (true === $showPreviousNext) {
    $pageResult = array_merge(
        $pageResult, 
        bootstrap_basic_fse_block_bsPagination_renderPageNext(($attributes ?? []), ($content ?? ''), ($block ?? new \WP_Block))
    );
...
if (empty($pageResult)) {
    ...
    $output = bootstrap_basic_fse_block_bsPagination_generateOutputHTML(($attributes ?? []), ($content ?? ''), ($block ?? new \WP_Block), $pageResult);
    unset($pageResult);
    ...
    printf(
        '<nav %1$s aria-label="' . esc_attr__('Pagination', 'bootstrap-basic-fse') . '">%2$s</nav>',
        $wrapper_attributes,
        $output
    );

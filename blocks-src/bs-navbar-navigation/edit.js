...
export default function Edit({ attributes, setAttributes }) {
    ...
    const navigationOptions = [
        {
            label: __('Auto: first available', 'bootstrap-basic-fse'),
            value: 0
        }
    ];
    ...
    if (Array.isArray(navigationPosts)) {
        navigationPosts.forEach((post) => {
            const title = post?.title?.rendered ? decodeEntities(post.title.rendered) : __('(no title)', 'bootstrap-basic-fse');
    ...
    return (
        <>
            <InspectorControls>
                <ToolsPanel
                    label={__('Navigation Settings', 'bootstrap-basic-fse')}
                ...
                >
                    <ToolsPanelItem
                        ...
                        label={__('Navigation', 'bootstrap-basic-fse')}
                    ...
                    >
                        ...
                        {navigationPosts !== null && (
                            <SelectControl
                                ...
                                help={__('Select a Navigation post or leave auto to use the first available.', 'bootstrap-basic-fse')}
                            />
                        )}
                        <div>
                            <ExternalLink href={createNavigationUrl}>
                                {__('Create a new navigation', 'bootstrap-basic-fse')}
                            </ExternalLink>
                        </div>
                        <div>
                            <ExternalLink href={manageNavigationUrl}>
                                {__('Manage all navigations', 'bootstrap-basic-fse')}
                            </ExternalLink>
                        </div>
                        {Array.isArray(navigationPosts) && navigationPosts.length === 0 && (
                            <Notice status="warning" isDismissible={false}>
                                {__('No Navigation posts found. Create one in the Site Editor > Navigation.', 'bootstrap-basic-fse')}
                            </Notice>
                        )}
                    </ToolsPanelItem>
                    ...
                    <ToolsPanelItem
                        ...
                        label={__('Dropdown menu class', 'bootstrap-basic-fse')}
                    ...
                    >
                        <TextControl
                            ...
                            help={__('Additional classes for dropdown menu. The base class `dropdown-menu` is always applied.', 'bootstrap-basic-fse')}
                        />
                    </ToolsPanelItem>
                </ToolsPanel>
            </InspectorControls>
                ...
                {Array.isArray(navigationPosts) && navigationPosts.length === 0 ? (
                    <div className="nav-item">
                        <span className="nav-link">
                            {__('No Navigation posts found to preview.', 'bootstrap-basic-fse')}
                        </span>
                    </div>
                ) : (
                    <Disabled>
